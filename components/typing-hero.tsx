...
function MobileTile({ cat, active, i }: { cat: typeof CATEGORIES[0]; active: boolean; i: number }) {
  return (
    <div
      ...
      style={{
        ...
        border: active ? `1.5px solid ${cat.glow}55` : "1px solid rgba(255,255,255,0.08)",
        boxShadow: active ? `0 0 20px ${cat.glow}18` : "none",
      }}
    >
      {active && (
        <div className="absolute inset-0 pointer-events-none" style={{ background: `radial-gradient(ellipse 80% 60% at 50% 100%, ${cat.glow}15, transparent 70%)` }} />
      )}
      <motion.img
        src={cat.icon}
        alt={cat.label}
        ...
        style={{ width: 52, height: 52, objectFit: "contain", position: "relative" }}
      />
      <p className="text-white text-xs font-semibold text-center leading-tight">{cat.label}</p>
      <p className="text-gray-500 text-[10px] text-center leading-tight">{cat.desc}</p>
      {active && <div className="absolute top-2 right-2 h-1.5 w-1.5 rounded-full animate-pulse" style={{ background: cat.glow }} />}
    </div>
  );
}
...
export default function TypingHero() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setSubIndex((i) => i + (deleting ? -1 : 1)), deleting ? 40 : 80);
    return () => clearTimeout(t);
  ...
  return (
    ...
    >
      <div className="flex flex-col lg:flex-row lg:absolute lg:inset-0 items-center gap-6 px-6 sm:px-10 lg:px-20 pt-32 pb-12 lg:pt-0 lg:pb-0">
        ...
        <div className="flex flex-col justify-center w-full lg:w-1/2 shrink-0">
          ...
          >
            <span className="h-1.5 w-1.5 rounded-full bg-[#37ca37] animate-pulse" />
            AI-Powered Agency
          </motion.div>
            ...
            <span className="inline-flex items-center gap-3 flex-wrap mt-1">
              ...
              >
                ...
                <span
                  ...
                  style={{ width: "0.06em", height: "0.75em", background: "currentColor" }}
                />
              </span>
            </span>
          </motion.h1>
          ...
          <motion.p
            ...
            style={{ fontSize: "clamp(0.9rem, 1.1vw, 1.05rem)", maxWidth: "40ch" }}
          >
            We build the automation, AI, and systems that scale your business — done for you.
          </motion.p>
          ...
          >
            <a
              ...
              style={{ background: "linear-gradient(135deg,#37ca37,#188bf6)", boxShadow: "0 0 24px rgba(55,202,55,0.3)" }}
            >
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3 mb-3">
            {CATEGORIES.slice(0, 3).map((cat, i) => (
              <MobileTile key={cat.label} cat={cat} active={i === wordIndex} i={i} />
            ))}
          </div>
          {/* row 2 — 2 tiles full width */}
          <div className="grid grid-cols-2 gap-3">
            {CATEGORIES.slice(3).map((cat, i) => (
              <MobileTile key={cat.label} cat={cat} active={3 + i === wordIndex} i={3 + i} />
            ))}
          </div>
        </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div
                ...
                style={{ background: `radial-gradient(ellipse 80% 60% at 50% 100%, ${active.glow}18, transparent 70%)` }}
              />
              <div>
                <span
                ...
                <p className="text-white font-semibold text-lg leading-snug">{active.label}</p>
              ...
              >
                <div
                  ...
                  style={{ width: 80, height: 20, background: active.glow, opacity: 0.5 }}
                />
                <img src={active.icon} alt={active.label} style={{ width: 110, height: 110, objectFit: "contain" }} />
              </motion.div>
            </motion.div>
          </AnimatePresence>

          {/* 4 smaller cards — the other categories */}
          {CATEGORIES.filter((_, i) => i !== wordIndex).slice(0, 4).map((cat, i) => (
            <motion.div
              key={cat.label}
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              >
                <div
                  ...
                  style={{ width: 30, height: 8, background: cat.glow, opacity: 0.45 }}
                />
                <img src={cat.icon} alt={cat.label} style={{ width: 48, height: 48, objectFit: "contain" }} />
              </motion.div>
              <div className="min-w-0">
                <p className="text-white text-sm font-medium truncate">{cat.label}</p>
                <p className="text-gray-500 text-xs truncate mt-0.5">{cat.desc}</p>
              </div>
              {/* subtle glow dot */}
              <div className="absolute top-3 right-3 h-1.5 w-1.5 rounded-full" style={{ background: cat.glow, opacity: 0.6 }} />
            </motion.div>
          ))}
        </motion.div>
      </div>
